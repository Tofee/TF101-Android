...
static int __init hw_setup(char *opt)
{
	...
	picasso_hw = simple_strtoul(opt, NULL, 0);
	return 0;
}
__setup("hw=", hw_setup);
...
static struct resource picasso_bcm4329_rfkill_resources[] = {
	{
...
static struct platform_device picasso_bcm4329_rfkill_device = {
	.name = "bcm4329_rfkill",
	.id             = -1,
	.num_resources  = ARRAY_SIZE(picasso_bcm4329_rfkill_resources),
	.resource       = picasso_bcm4329_rfkill_resources,
};

static struct resource picasso_bluesleep_resources[] = {
	[0] = {
		.name = "gpio_host_wake",
...
static struct platform_device picasso_bluesleep_device = {
	.name           = "bluesleep",
	.id             = -1,
	.num_resources  = ARRAY_SIZE(picasso_bluesleep_resources),
	.resource       = picasso_bluesleep_resources,
};

static void __init picasso_setup_bluesleep(void)
{
	platform_device_register(&picasso_bluesleep_device);
...
static struct tegra_ulpi_config picasso_ehci2_ulpi_phy_config = {
	.reset_gpio = TEGRA_GPIO_PV1,
	.clk = "cdev2",
};

static struct tegra_ehci_platform_data picasso_ehci2_ulpi_platform_data = {
	.operating_mode = TEGRA_USB_HOST,
	.power_down_on_bus_suspend = 1,
	.phy_config = &picasso_ehci2_ulpi_phy_config,
...
static struct wm8903_platform_data picasso_wm8903_pdata = {
	...
	.micdet_cfg = 0x83,
	.micdet_delay = 100,
	.gpio_base = PICASSO_GPIO_WM8903(0),
	.gpio_cfg = {
...
static struct fm34_platform_data picasso_fm34_pdata = {
	.gpio_reset	= TEGRA_GPIO_PH2,
	.gpio_en	= TEGRA_GPIO_PH3,
};

static const struct i2c_board_info picasso_dsp_board_info[] = {
	{
		I2C_BOARD_INFO("fm34", 0x60),
		.platform_data = &picasso_fm34_pdata,
	},
};

static void __init picasso_dsp_init(void)
{
	i2c_register_board_info(0, picasso_dsp_board_info,
			ARRAY_SIZE(picasso_dsp_board_info));
}
...
static void picasso_usb_init(void)
{
	...
	tegra_ehci2_device.dev.platform_data
		= &picasso_ehci2_ulpi_platform_data;
...
unsigned int ASUS3GAvailable()
{
	...
	if (!HW_DRF_VAL(TEGRA_DEVKIT, MISC_HW, PROJECT, ventana_hw)) {
		//Only PICASSO has 3G SKU definition
...
unsigned int ASUSWiMAXAvailable()
{
	...
	if (!HW_DRF_VAL(TEGRA_DEVKIT, MISC_HW, PROJECT, ventana_hw)) {
		//Only PICASSO has WiMAX SKU definition
...
unsigned int ASUSCheckWLANVendor(unsigned int vendor)
{
	...
	if (HW_DRF_VAL(TEGRA_DEVKIT, MISC_HW, PROJECT, ventana_hw) < 2) {
		//All valid projects (PICASSO/SL101) have BT/WLAN module
		//definition
		switch (vendor) {
...
unsigned int ASUSCheckTouchVendor(unsigned int vendor)
{
	...
	if (HW_DRF_VAL(TEGRA_DEVKIT, MISC_HW, PROJECT, ventana_hw) < 2) {
		//All valid projects (PICASSO/SL101) have touch panel module
		//definition
		switch (vendor) {
...
static void ASUSGetProjectName()
{
	...
	if(ret == 0) { //PICASSO
		if(ASUS3GAvailable()) {
			strcpy(ventana_projectname,"PICASSOG");
		}
		else {
			if(ASUSWiMAXAvailable())
				strcpy(ventana_projectname,"PICASSO-WiMAX");
			else
				strcpy(ventana_projectname,"PICASSO");
		}
	}
	else if(ret == 1) { //SL101
...
static void ASUSGetPcbid()
{
	...
	unsigned int ret = 0;
	ret = HW_DRF_VAL(TEGRA_DEVKIT, MISC_HW, PROJECT, ventana_hw);
	if(ret == 0 || ret ==1) { //PICASSO or SL101
	       pcbid = ventana_hw >> 4;
